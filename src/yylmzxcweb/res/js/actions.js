/* ============================================================
   ...
   只管「编辑导航内容」这一件事（分组 / 书签 / 站点信息的增删改排序）：
      · 本模块是数据“内容”的唯一写入方；state 容器与标志位由 store 独占（见 store.js）；
...
(function (Nav) {
  ...
  function setEditing(on) {
    ...
    if (on && !store.canEdit()) {
      dom.toast('编辑需要连接数据库：请先启动「启动主页(带数据库).bat」，再把模式切到数据库');
      return false;
    }
    store.setEditing(on);
  ...
  function backgroundPicker(inp, refresh) {
    ...
    pick.addEventListener('click', function () {
      dom.pickFile('image/*', function (file) {
        pick.disabled = true;
        dom.toast('正在上传图片 ...');
        // 读文件（浏览器副作用）交给 dom，HTTP 上传交给 api，各自职责单一
        dom.readAsDataURL(file).then(api.uploadImage).then(function (path) {
          inp.value = path;
          syncThumb();
          if (refresh) refresh();
    ...
    modal.open({
      ...
      fields: [
        ...
        { key: 'name', label: '昵称', value: p.name || '', placeholder: '头像加载失败时的备用文字' },
        {
          ...
          placeholder: '留空用默认头像 res/img.png；也可填自定义头像链接'
        },
        {
