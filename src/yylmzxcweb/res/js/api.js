...
(function (Nav) {
  ...
  var BACKEND_HINT = '请先运行「启动导航站(带数据库).bat」把后端跑起来；' +
  ...
  // 健康检查（3 秒超时）；只回答「能不能连通」，不抛异常
  function health() {
    return healthInfo().then(function (info) { return !!(info && info.ok); });
  }
  ...
  // 上传本机图片（背景图）：读成 data URL 交给后端落盘，回传可直接写进数据的相对路径
  ...
  // 文件 → data URL。浏览器副作用，只在本模块内部使用
  function readAsDataURL(file) {
    return new Promise(function (resolve, reject) {
      var reader = new FileReader();
      reader.onload = function () { resolve(String(reader.result || '')); };
      reader.onerror = function () { reject(fail('READ', '读取图片失败')); };
