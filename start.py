...
class UnifiedServer(http.server.BaseHTTPRequestHandler):
    def do_OPTIONS(self):
    ...
    def do_GET(self):
        ...
        if '/proxy.php' in path or '/proxy' in path:
...
def main():
    server = http.server.HTTPServer(('0.0.0.0', PORT), UnifiedServer)
    print('=' * 60)
    print('  SC Web - 本地开发服务器')
    print('  (静态文件 + API 代理)')
    print('=' * 60)
    print(f'  网站地址: http://localhost:{PORT}/online_server.html')
    print(f'  代理接口:  http://localhost:{PORT}/proxy.php')
    print(f'  服务器列表: http://localhost:{PORT}/proxy.php?action=serverlist')
    print(f'  Ping检测:  http://localhost:{PORT}/proxy.php?action=ping&host=xxx&port=xxx')
    print('=' * 60)
